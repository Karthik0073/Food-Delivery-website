...
var foodItem = [];
fetch('/api/v1/getfooditems').then(res => res.json()).then(res => {
    const promises = res.map(fItem => {
        return fetch(`/api/v1/getfooditem/${fItem}`)  
    });


    Promise.allSettled(promises).then(async results => {
        const promiseResults = await Promise.all(results.map(async (res, index) => {
            const url = res.value.url;
            const dataRes = await res.value.json();
            return dataRes.map((item, fIndex) => {
                const foodCategory = url.substring(url.lastIndexOf("/")+1);
                return ({
                    ...item,
                    img: 'images/'+foodCategory+"/"+ item.img,
                    quantity: 1,
                    category: foodItemMapping[foodCategory].category
                });
            });
        }));

        promiseResults.map(res => foodItem.push(...res));
